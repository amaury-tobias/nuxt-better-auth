...
Create `auth.config.ts` in project root exporting `defineServerAuth(...)`.

## "Missing client-auth.config.ts"

The module expects a client config file at `client-auth.config.ts` (or `app/client-auth.config.ts` for Nuxt 4) exporting `createAppAuthClient(baseURL)`.
...
- Confirm the file name is exactly `auth.config.ts` in your project root.
