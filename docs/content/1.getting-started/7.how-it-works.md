...
::steps
### 1. Initialization
During build, the module reads `auth.config.ts` and generates the auth handler. If using NuxtHub, it also generates the database schema automatically.

### 2. Server Runtime
The module injects a catch-all API handler at `/api/auth/**`. This handler processes all authentication requests (login, logout, session fetching) using the configuration provided in `auth.config.ts`.
